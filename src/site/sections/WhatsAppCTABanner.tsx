import { motion } from "framer-motion";
import { waLink } from "@/site/lib/utils";

export default function WhatsAppCTABanner() {
  return (
    <section className="bg-blaze topo-texture py-16 lg:py-20 overflow-hidden">
      <style>{`.topo-texture::before { color: #ffffff; opacity: 0.08; }`}</style>

      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">

        {/* Floating icons */}
        <motion.span
          animate={{ y: [0, -10, 0] }}
          transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }}
          className="absolute left-8 top-0 text-4xl opacity-20 hidden lg:block select-none"
          aria-hidden
        >⛺</motion.span>

        <motion.span
          animate={{ y: [0, -8, 0] }}
          transition={{ repeat: Infinity, duration: 3.5, delay: 0.5, ease: "easeInOut" }}
          className="absolute right-12 top-4 text-3xl opacity-20 hidden lg:block select-none"
          aria-hidden
        >🧭</motion.span>

        <motion.span
          animate={{ y: [0, -12, 0] }}
          transition={{ repeat: Infinity, duration: 4, delay: 1, ease: "easeInOut" }}
          className="absolute right-32 bottom-2 text-3xl opacity-15 hidden lg:block select-none"
          aria-hidden
        >🔥</motion.span>

        {/* Headline */}
        <motion.h2
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          className="font-display font-black text-white text-3xl lg:text-5xl leading-tight mb-4"
        >
          250+ families planned their trip<br />
          <span className="italic">exactly this way.</span>
        </motion.h2>

        <motion.p
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ delay: 0.2 }}
          className="font-body font-bold text-white text-base lg:text-lg mb-10"
        >
          One conversation. A real expert. Your trip, planned around you.
          <br />
          <span className="text-white/90 font-normal text-sm">Responds within 2 hrs · Mon–Sat, 9am–9pm IST · No spam, no pressure.</span>
        </motion.p>

        {/* CTA */}
        <motion.div
          initial={{ opacity: 0, scale: 0.9 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          transition={{ delay: 0.3, type: "spring", damping: 18 }}
        >
          <a
            href={waLink({ source: "home_cta_banner" })}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 bg-abyss text-white font-display font-bold text-lg px-10 py-5 rounded-full shadow-xl hover:shadow-2xl hover:bg-drift active:scale-[0.97] transition-all duration-200"
          >
            <svg className="w-6 h-6 text-[#25D366]"  viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
            Plan My Trip on WhatsApp
          </a>
        </motion.div>
      </div>
    </section>
  );
}
