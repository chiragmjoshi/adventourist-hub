...
const steps = [
  {
    ...
    icon: (
      <svg className="w-7 h-7"  viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
  ...
  return (
    <section className="bg-horizon topo-texture py-20 lg:py-28 overflow-hidden">
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <p className="font-body text-xs font-semibold uppercase tracking-widest text-abyss/50 mb-3">
            How It Works
          </p>
        ...
        >
          ...
          <div className="hidden lg:block absolute top-[3.25rem] left-[calc(16.67%+2rem)] right-[calc(16.67%+2rem)] h-px border-t-2 border-dashed border-abyss/20 z-0" />
          ...
          {steps.map((step) => (
            ...
            >
              {/* Number badge */}
              <div className="w-16 h-16 rounded-full bg-abyss flex items-center justify-center mb-6 shadow-lg shadow-black/20">
                <span className="font-display font-black text-white text-xl">{step.number}</span>
              </div>

              {/* Icon */}
              <div className="text-abyss/60 mb-4">
                {step.icon}
              </div>

              <h3 className="font-display font-bold text-abyss text-xl mb-3">
                {step.title}
              </h3>
              <p className="font-body text-abyss/60 text-sm leading-relaxed max-w-xs">
                {step.desc}
              </p>
            </motion.div>
          ))}
        ...
        >
          <a
            href={`https://wa.me/${WHATSAPP_NUMBER}?text=Hi!%20I%27d%20like%20to%20start%20planning%20a%20trip.`}
            target="_blank"
